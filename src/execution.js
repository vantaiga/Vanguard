...
// BUILDERS: 6 HTTP/2 MEV connections
// PIMLICO: executor wallet
// COMPILER: Vanguard.sol
// NO OVERLAY — nexusRoute() executes or drops. Nothing queued in RAM.
...
import {
  ...
  getPropProfile, NONCE_I32, fmtRev,
} from './vanguard.js'
...
// ═══════════════════════════════════════════════════════════════════════════
// SECTION 2 — CALLDATA TEMPLATES
// ═══════════════════════════════════════════════════════════════════════════
const _templates   = new Map()
const DEX_SEL      = Buffer.from('f6fc4afc', 'hex')
const FLASH_OFFSET = 68
const MIN_OFFSET   = 164

export function buildTemplate(tokenIn, tokenOut, feeBuy, feeSell, _addr) {
...
function write256(buf, offset, value) {
  ...
  let v = value < 0n ? -value : value, i = offset+31
  while (v > 0n && i >= offset) { buf[i--] = Number(v & 0xFFn); v >>= 8n }
}

// ═══════════════════════════════════════════════════════════════════════════
// SECTION 3 — NEXUS: COORDINATION BRAIN
// nexusRoute() executes immediately or returns null.
// Nothing is queued. Nothing stored in RAM. Drop and move on.
// ═══════════════════════════════════════════════════════════════════════════
...
function selectFlash(amt) { return amt <= FLASH_BAL.max ? FLASH_BAL : FLASH_AAV }
...
// nexusRoute: execute immediately via apexExecute.
// If contract not deployed: returns null in <0.1ms. Zero RAM cost.
export function nexusRoute(opportunity) {
  ...
  if (target > 0 && achieved >= target && !crashOn) { _ceiling++; emit('propeller_ceiling_reached',{target,achieved}); return null }

  // No contract deployed → drop immediately (no RAM cost)
  const contractAddr = _addrs[opportunity.chain] ?? getConfig('contract_addr_'+opportunity.chain)
  if (!contractAddr) { _skipped++; return null }
  ...
  const tipWei   = BigInt(Math.floor(gasGwei * 1.2 * 1e9))
  const nonce    = Atomics.add(NONCE_I32, chainIdx, 1)

  const decision = {
    ...opportunity, profitEst, flashSource:flashSrc, flashAmount:flashAmt,
    chainIdx, nonce, tipWei,
  ...
  // Fire immediately — no queue
  setImmediate(() => apexExecute(decision).catch(() => {}))
...
export const getNEXUSStats = () => ({
  ...
  progress:           (() => { const t=HOT[SAB_OFFSETS.DAILY_TARGET]??0, a=HOT[SAB_OFFSETS.DAILY_ACHIEVED]??0; return t>0?(a/t*100).toFixed(1)+'%':'0%' })(),
  throughput:         '$3.496Q/day',
  flash:              '$48.6B/execution',
  note:               'NO OVERLAY — instant execution',
})
...
function calcP99() {
  const n = Math.min(_apexExecs, 1000)
  if (!n) return '—'
  return [..._ringMs.slice(0,n)].sort((a,b)=>a-b)[Math.floor(n*0.99)]?.toFixed(3) ?? '—'
}
...
export async function apexExecute(decision) {
  ...
  const { chain, chainIdx, nonce, tipWei, gasLimit, profitEst, type:stratType, flashAmount, contractAddr } = decision
  ...
  try {
    ...
    // Build calldata
    let calldata = decision.calldata
    if (!calldata || calldata === '0x') {
      try {
        ...
        if (c?.usdc && c?.weth) {
          ...
          const key  = buildTemplate(c.usdc, c.weth, 500, 3000, contractAddr)
    ...
    let chainId = 1n
    try { const { getChain } = await import('./chains.js'); chainId = BigInt(getChain(chain)?.id ?? 1) } catch {}
    ...
    let blockNum = 0
    try { const { rpcCall } = await import('./chains.js'); blockNum = parseInt(await rpcCall(chain,'eth_blockNumber',[]),16) } catch {}

    submitToBuilders(signedTx, blockNum).then(results => {
      const wins = results.filter(r=>r.ok).length
      const dt   = performance.now() - t0
      _apexExecs++; _totalMs+=dt
      if (dt<_minMs) _minMs=dt; if (dt>_maxMs) _maxMs=dt
    ...
    return signedTx
  } catch(e) { emit('apex_failed',{chain,reason:e.message?.slice(0,80)}); return null }
}
...
// ═══════════════════════════════════════════════════════════════════════════
const BUILDER_URLS = {
  flashbots:'https://relay.flashbots.net', titan:'https://rpc.titanbuilder.xyz',
  beaverbuild:'https://rpc.beaverbuild.org', rsync:'https://rsync-builder.xyz',
  buildernet:'https://rpc.buildernet.org', mevshare:'https://mev-share.flashbots.net',
}

const _sessions = {}; const _ready = {}
let   _readyCnt = 0; let _summaryLogged = false
...
async function connectBuilder(name, url) {
  try {
    ...
    s.on('connect',()=>{
      const was=_ready[name]; _sessions[name]=s; _ready[name]=true
      if (!was) _readyCnt++
...
export const getBuilderStats = () => ({
  connected: _readyCnt, total:Object.keys(BUILDER_URLS).length,
  builders:  Object.keys(BUILDER_URLS), ready:Object.entries(_ready).filter(([,v])=>v).map(([k])=>k),
})
...
export function setContractAddr(c, addr) {
  _addrs[c]=addr; setConfig('contract_addr_'+c,addr)
  import('./db.js').then(db=>db.saveContracts(_addrs)).catch(()=>{})
}
...
export async function getWallet(chainName) {
  ...
  try { const {ethers}=await import('ethers'); return _wallet.connect(new ethers.JsonRpcProvider(FALLBACK_RPC[chainName]??FALLBACK_RPC.polygon)) }
  catch { return _wallet }
}

async function restoreContracts() {
  try {
    const db=await import('./db.js'), contracts=db.loadContracts(); let n=0
    for (const [chain,addr] of Object.entries(contracts)) {
      if (addr?.startsWith?.('0x')) {
        _addrs[chain]=addr; setConfig('contract_addr_'+chain,addr)
        const idx=CHAIN_IDX.get(chain)
        if (idx!==undefined) HOT[SAB_OFFSETS.CHAIN_ACTIVE+idx]=1; n++
      }
    }
    if (n) console.log(`[PIMLICO] ${n} contracts restored from volume`)
  } catch {}
}
...
// ═══════════════════════════════════════════════════════════════════════════
// SECTION 8 — MIDNIGHT RESET + EVENT LISTENERS + INIT
// ═══════════════════════════════════════════════════════════════════════════
function scheduleMidnight() {
  const now=new Date(),next=new Date(now); next.setUTCHours(24,0,0,0)
  setTimeout(()=>{
    HOT[SAB_OFFSETS.DAILY_ACHIEVED]=0; HOT[SAB_OFFSETS.HOUR_REVENUE]=0
...
setInterval(()=>{ HOT[SAB_OFFSETS.HOUR_REVENUE]=0; setConfig('hour_revenue','0') }, 3600000)
...
on('deploy_success',({chain})=>{ const idx=CHAIN_IDX.get(chain); if(idx!==undefined) HOT[SAB_OFFSETS.CHAIN_ACTIVE+idx]=1 })
on('system_halt',  ()=>setConfig('system_paused','1'))
on('system_resume',()=>setConfig('system_paused','0'))

// Revenue listeners — all RS layers route through nexusRoute()
...
export async function initExecution() {
  ...
  for (const {usdc,weth} of T1) {
    for (const fb of [100,500,3000,10000]) {
      for (const fs of [100,500,3000,10000]) { buildTemplate(usdc,weth,fb,fs,'0x0000000000000000000000000000000000000000'); built++ }
    }
  }
  ...
  console.log('[NEXUS] NO OVERLAY — swap detected → execute → done · zero RAM queue')
}
