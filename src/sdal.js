// Vanguard · sdal.js
// Software-Defined Abstraction Layer.
// ALL Vanguard parameters live here. Runtime-configurable. No redeploy.
// SharedArrayBuffer cache: zero-cost reads on hot path.
// EXPORTS EVERY NAME any file imports — nothing missing.

import { readFileSync, writeFileSync, existsSync, mkdirSync } from 'fs'

// ── SharedArrayBuffer — hot path reads cost 1 CPU cycle ──────────────────────
const _SAB = new SharedArrayBuffer(2048)
const _F64 = new Float64Array(_SAB)

// ── SAB layout — exported so every file can read hot-path values ──────────────
export const SAB_OFFSETS = {
  PROPELLER:      0,
  CRASH_SCORE:    1,
  MIN_PROFIT:     2,   // +chainIndex (0-19)
...
// ── State ─────────────────────────────────────────────────────────────────────
let _data = JSON.parse(JSON.stringify(DEFAULT))
...
function load() {
  try {
    if (existsSync(SDAL_PATH)) {
      _data = { ...DEFAULT, ...JSON.parse(readFileSync(SDAL_PATH, 'utf8')) }
    } else {
      save()
    }
  } catch(e) {
    console.warn('[SDAL] Using defaults:', e.message?.slice(0, 60))
    _data = JSON.parse(JSON.stringify(DEFAULT))
  }
}

function save() {
  try { mkdirSync('/data', { recursive:true }); writeFileSync(SDAL_PATH, JSON.stringify(_data, null, 2)) } catch {}
}

// ── SAB sync ──────────────────────────────────────────────────────────────────
function syncSAB() {
  try {
    const p    = parseInt(_F64[SAB_OFFSETS.PROPELLER] || 5)
    ...
    if (!_F64[SAB_OFFSETS.PROPELLER]) _F64[SAB_OFFSETS.PROPELLER] = 5
  } catch {}
}

// ── Public API ────────────────────────────────────────────────────────────────
export function get(key)           { return _data[key] ?? null }
export function getAddr(name)      { return _data.protocol_addresses?.[name] ?? null }
export function getChainCfg(name)  { return _data.chain_configs?.[name] ?? null }
export function getStrategy()      { return _data.strategy_params ?? DEFAULT.strategy_params }
export function getEnergy()        { return _data.energy_config   ?? DEFAULT.energy_config }
export function getThroughput()    { return _data.throughput       ?? DEFAULT.throughput }
export function getV7()            { return _data.v7_config        ?? DEFAULT.v7_config }

export function getPropProfile(p) {
  const level = Math.max(1, Math.min(30, Math.round(Number(p) || 5)))
  return _data.propeller_profiles?.[level] ?? DEFAULT.propeller_profiles[5]
}
...
export function initSDAL() {
  load()
  syncSAB()
  setInterval(() => { save(); syncSAB() }, 60000)
  console.log(`[SDAL] v${_data.version} — ${Object.keys(_data.protocol_addresses).length} addresses`)
